...
uint32_t possibleEncodings(const char* s)
{
    ...
    while ((ch1 = *chp++)) {
        if (ch1 & 0x80) {
            ...
            // LOGD("1 result = %u", result);
            uint32_t re = findPossibleEncodings(ch);
            if (re != kEncodingNone) {
                result &= re;
            }
            // LOGD("2 result = %u", result);
        }
...
bool CharacterEncoder::is_ascii(const char *str) {
    unsigned char chr;
...
bool CharacterEncoder::is_non_ascii_UTF8(const char* str, int &len) {
    if (*str == 0) {
        return false;
    }
    ...
    if (chr >> 5 == two_byte) {
        len = 2;
    } else if (chr>>4 == three_byte) {
        len = 3;
    } else if (chr>>3 == four_byte) {
        len = 4;
    } else if (chr>>2 == five_byte) {
        len = 5;
    } else if (chr>>1 == six_byte) {
        len = 6;
    } else {
        return false;
    }
...
bool CharacterEncoder::is_utf8_code(const char* str, int *wordcount)
{
    unsigned char chr;
    int utf8_count = 0;
    int non_utf8_count = 0;
...
#include <stdlib.h>
bool CharacterEncoder::is_gbk_code(const char *str, int *wordcount) {
    ...
    if (gbk_no > 0) return false;

    return true;
}

bool CharacterEncoder::isUTF8(const char *str) {
    if (is_ascii(str)) {
        return true;
    }
    ...
    if (!is_utf8_code(str, &words1)) {
        return false;
    }

    if (!is_gbk_code(str, &words2)) {
        return true;
    }
